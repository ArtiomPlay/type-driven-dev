...
data Vec : Nat -> Type -> Type where
    Nil : Vec 0 elem
    (::) : elem -> Vec len elem -> Vec (S len) elem
...
processInput x "printAll" = Just (case x of
                                       (MkData size items) => show items, x)
processInput x cmd = case unpack cmd of
                          case_val => ?add
processInput x _ = Nothing

covering
main : IO ()
main = replWith (MkData 1 ["Labas"]) "\n>>>" processInput
